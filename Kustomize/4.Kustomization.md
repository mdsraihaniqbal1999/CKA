...
## 📂 Directory Structure
Organizing your Kubernetes manifests properly is crucial for **scalability and maintainability**. Below is the recommended directory structure for using Kustomize:
...
│   │   ├── deployment-patch.yaml
```
- **`base/`**: Contains the standard Kubernetes manifests.
- **`overlays/dev/`**: Custom configurations for the development environment.
- **`overlays/prod/`**: Custom configurations for the production environment.
...
To see how Kustomize builds the final manifest before applying it, run:
```sh
kustomize build k8s/overlays/dev/
```
This will output the fully rendered YAML files with applied overlays and transformations.
...
- **Use `kustomize build` to preview configurations** before applying them.
